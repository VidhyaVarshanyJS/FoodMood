...
res_data = res_data.loc[(res_data['Country Code'] == 1)
                        & (res_data['City'] == 'New Delhi'), :]
res_data = res_data.loc[res_data['Longitude'] != 0, :]
res_data = res_data.loc[res_data['Latitude'] != 0, :]
# clearing out invalid outlier
...
def search_comfort(mood):
    ...
    for i in range(124):
        temp = [temps.strip().replace('.', '').replace(',', '').lower() for temps in str(
            food_data["comfort_food_reasons"][i]).split(' ') if temps.strip() not in stop]
        if mood in temp:
            foodtemp = [lemmatizer.lemmatize(temps.strip().replace('.', '').replace(',', '').lower(
            )) for temps in str(food_data["comfort_food"][i]).split(',') if temps.strip() not in stop]
            for a in foodtemp:
...
def find_my_comfort_food(mood):
    topn = []
    # function create dictionary only for particular mood
...
def find_restaurant():
    ...
    result_str = 'You should eat {}, {}, or {}.'.format(
    ...
    restaurants_list = []
    for item in result:
        restaurants = res_data[res_data.Cuisines.str.contains(
            food_to_cuisine_map[item], case=False)].sort_values(by='Aggregate rating', ascending=False).head(3)
